...
function AdminPage(){
  ...
  const run=async()=>{
    setLoading(true);
    try{ const r=await fetch(`/api/hunter?city=${city}`); setData(await r.json()); }catch(e){ setData({error:"أضف ملفات api"}); }
    setLoading(false);
  };
  return(
    <div dir="rtl" style={{background:"#0A0A0B",minHeight:"100vh",color:"white",padding:24,fontFamily:"Tajawal"}}>
      <h1 style={{fontSize:28,fontWeight:900}}>🛰️ لوحة تحكم 4 خوادم AI - سرية</h1>
      <p style={{color:"#888"}}>لك وحدك - الزبائن لا يرونها</p>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:16,marginTop:24}}>
        ...
        <div style={{background:"#151517",border:"1px solid #222",borderRadius:16,padding:16}}><h3>🛡️ Verifier AI</h3><p style={{fontSize:12,color:"#888"}}>يفحص حقيقي/مزيف تلقائيا</p><div style={{marginTop:8,background:"#000",padding:8,borderRadius:8,fontSize:12}}>✅ يكشف المزيفين<br/>✅ يعطي Trust Score</div></div>
        <div style={{background:"#151517",border:"1px solid #222",borderRadius:16,padding:16}}><h3>💰 Pricer AI</h3><p style={{fontSize:12,color:"#888"}}>يسعر عادل + عمولتك</p><div style={{marginTop:8,background:"#000",padding:8,borderRadius:8,fontSize:12}}>✅ ثمن عادل<br/>✅ عمولتك 12% محسوبة</div></div>
        <div style={{background:"#151517",border:"1px solid #222",borderRadius:16,padding:16}}><h3>🤝 Matcher AI</h3><p style={{fontSize:12,color:"#888"}}>يربط شاري ببائع</p><a href={TELEGRAM_LINK} target="_blank" style={{display:"block",textAlign:"center",background:"white",color:"black",padding:10,borderRadius:999,textDecoration:"none",fontWeight:"bold",marginTop:8}}>فتح تليجرام</a></div>
      </div>
      <a href="/" style={{display:"inline-block",marginTop:24,color:"#00D084"}}>← العودة للموقع</a>
    </div>
  );
}
  ...
  return(
    <div dir="rtl" style={{fontFamily:"Tajawal, sans-serif",background:"#0A0A0B",minHeight:"100vh",color:"white"}}>
      ...
      <section style={{padding:"60px 24px",maxWidth:1100,margin:"0 auto"}}>
        ...
        <h1 style={{fontSize:"clamp(32px,6vw,52px)",fontWeight:900,lineHeight:1.1,margin:0}}>لا تبدأ من <span style={{color:"#00D084"}}>الصفر،</span><br/>اشتر متجراً يشتغل</h1>
        <p style={{color:"rgba(255,255,255,0.6)",maxWidth:500,marginTop:16,fontSize:16,lineHeight:1.6}}>4 خوادم ذكاء اصطناعي تفحص وتسعر وتطابق تلقائيا - مجانية ولا تبطئ الموقع. نحن نجد لك المتاجر الميتة في كازا ونفحصها لك.</p>
        <div style={{display:"flex",gap:12,marginTop:24,flexWrap:"wrap"}}>
          ...
          <a href={TELEGRAM_LINK} target="_blank" style={{background:"rgba(255,255,255,0.1)",border:"1px solid rgba(255,255,255,0.2)",color:"white",padding:"14px 20px",borderRadius:999,textDecoration:"none",fontWeight:"bold",display:"flex",alignItems:"center"[STRIPPED 33 bytes]"18" height="18" viewBox="0 0 24 24" fill="white"><path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm5.894 8.221l-1.97 9.28c-.145.658-.537.818-1.084.508l-3-2.21-1.447 1.394c-.16.16-.295.295-.605.295l.213-3.054 5.56-5.022c.24-.213-.054-.334-.373-.121l-6.869 4.326-2.96-.945c-.643-.2-.658-.643.135-.954l11.566-4.458c.538-.196 1.006.12.832.941z"/></svg>
            تواصل على تليجرام
          </a>
        </div>
      </section>
      ...
      <section id="stores" style={{padding:"20px 24px",maxWidth:1100,margin:"0 auto"}}>
        <h2 style={{fontSize:24,fontWeight:900}}>متاجر جاهزة للبيع</h2>
        <p style={{fontSize:13,color:"rgba(255,255,255,0.4)",marginTop:4}}>سيتم استبدالها بمتاجر حقيقية من Hunter AI</p>
      ...
      <footer style={{textAlign:"center",padding:"30px 24px",fontSize:12,color:"rgba(255,255,255,0.3)",borderTop:"1px solid #222",marginTop:40}}>
        <div style={{maxWidth:1100,margin:"0 auto"}}>
          <p style={{fontSize:14,color:"white",fontWeight:"bold",marginBottom:8}}>Tajr.me - أول سوق للمتاجر الجاهزة في المغرب</p>
          <p>© 2026 Tajr.me - 4 خوادم AI نشطة - كازا | مدعوم بالذكاء الاصطناعي</p>
          <div style={{marginTop:12,display:"flex",justifyContent:"center",gap:12}}>
            <a href={TELEGRAM_LINK} target="_blank" style={{color:"#00D084",textDecoration:"none"}}>تليجرام</a>
            <span>•</span>
            <a href="/admin" style={{color:"#888",textDecoration:"none"}}>لوحة التحكم</a>
          </div>
        </div>
      </footer>
    </div>
...
export default function App(){
  ...
  useEffect(()=>{
    if(window.location.pathname.toLowerCase().includes("admin")||window.location.hash.toLowerCase().includes("admin")) setIsAdmin(true);
  },[]);
